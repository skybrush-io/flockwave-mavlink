...
class MAVLinkMessage(Protocol):
    ...
    def pack(self, mav, force_mavlink1: bool = False) -> bytes: ...
    def sign_packet(self, mav) -> None: ...
...
class MAVLinkSigningInterface(Protocol):
    ...
    link_id: int
    sign_outgoing: bool
    allow_unsigned_callback: Optional[Callable[[Any, int], bool]]
...
class MinimalMAVLinkInterface(Protocol):
    ...
    def parse_buffer(
        self, /, s: Sequence[int]
    ) -> Optional[Sequence[MAVLinkMessage]]: ...
